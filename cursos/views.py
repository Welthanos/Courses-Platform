...
def curso(request, id):
    if request.session.get('usuario'):
        aulas = Aula.objects.filter(curso = id)
        request_usuario = request.session.get('usuario')
...
def aula(request, id):
    if request.session.get('usuario'):
        ...
        request_usuario = request.session.get('usuario')
        usuario_avaliou = NotaAula.objects.filter(aula_id = id).filter(usuario_id = request_usuario)
...
def processa_avaliacao(request):
    if request.session.get('usuario'):
        ...
        usuario_avaliou = NotaAula.objects.filter(aula_id = aula_id).filter(usuario_id = usuario_id)
